...
def construct_latest_block_based_cache_middleware(
...
) -> Callable[[Callable[[RPCEndpoint, Any], RPCResponse], "aweb3.aWeb3"], Coroutine[Any, Any, AsyncMiddleware]]:
    """
    ...
    async def latest_block_based_cache_middleware(
        ...
        async def _update_block_info_cache() -> None:
            ...
            if avg_block_time_age_in_blocks >= avg_block_sample_size:
                ...
                ancestor_block = await web3.eth.get_block(ancestor_block_number)
            ...
            if 'latest_block' in block_info:
                ...
                if time_since_latest_block > avg_block_time:
                    block_info['latest_block'] = await web3.eth.get_block('latest')
            else:
